...
public class ChatController {
    ...
    @MessageMapping("/move")
    @SendTo("/move/player")
    public Helper movePlayer(@Payload(required = false) Helper character){

        // we need to test this out 
        if(character != null){
            character.setPosition(character.getNewPosition());
            
            System.out.println("----------" + character.getId());
    
        }

       // System.out.println("----------asdkaksd");
    ...
    // @MessageMapping("/hello")
    // @SendTo("/topic/greetings")
    // public ArrayList<Helper> receivedPrivateMessage(){
